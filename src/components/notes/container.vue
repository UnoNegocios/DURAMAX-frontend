...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      editItem(item){
        item.seen = true
        axios.put("https://backendduramax.unocrm.mx/api/v1/note/update",Object.assign(item)).then(response=>{
            this.$store.dispatch('note/getNotes')
        })
      },
      deleteNote(){
        axios.delete("https://backendduramax.unocrm.mx/api/v1/note/delete/"+this.deleteId).then(response => {
          this.deleteId = ''
          this.sheet = false
          this.$store.dispatch('note/getNotes')
        });
