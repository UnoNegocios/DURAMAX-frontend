        ...
        <div class="invoice-box" style="background-color:white;" id="divName" elevation="0">
            <table cellpadding="0" cellspacing="0" class="divName" style="max-width:644px!important;">
                <tr class="top">
                    <td colspan="4" class="pb-0">
                        <table>
                            <tr>
                                <td class="title" style="padding-bottom:0px!important;">
                                <img src="/duramax-logo.png" style="width:100%; max-width:100px;" class="pb-0">
                                </td>
                                <td style="font-size:15px; line-height:1.5em; padding-top:50px;" class="pb-0">
                                CON SEGURIDAD, EL CALZADO DE USO PROFESIONAL</br><strong style="color:#0e0b49;">CALZADODURAMAX.COM</strong>
                                </td>
                            </tr>
                        </table>
                    </td>
                </tr>
                <div style="height: 3px; background: #0e0b49; width: 644px; position: absolute;"></div>
                ...
                <tr class="information" style="font-size:14px; line-height:1.5em;">
                    <td colspan="4" style="padding-bottom:0px!important;">
                        ...
                        <tr>
                            ...
                            <br>
                            <strong style="font-size:15px;">ATN: {{currentUser.name}} {{currentUser.last}}</strong>
                            </td>
                            <td style="padding-bottom:0px!important;">
                            Monterrey, Nuevo León {{fecha(quotation.created_at)}}
                            </td>
                        </tr>
                        </table>
                    </td>
                ...
                <tr class="heading">
                    ...
                    <td style="text-align:center;">Precio de Lista</td>
                    <td style="text-align:center;">Precio Preferencial</td>
                </tr>
                <tr class="item" v-for="(item, index) in quotation.items" :key="index">
                    <td style="text-align:center;">
                    ({{item.quantity}})    {{itemDetail(item.item).name}}
                    <v-img :src="'https://backendduramax.unocrm.mx/files/' + itemDetail(item.item).image" style="width:100%; max-width:150px; margin:10px;"/>
                    </td>
                    <td style="text-align:center;">{{itemDetail(item.item).description}}</td>
                    <td style="text-align:center;">{{ (itemDetail(item.item).price*1).toLocaleString('es-MX', { style: 'currency', currency: 'MXN',}) }}*</td>
                    <td style="text-align:center;">{{(item.value*1).toLocaleString('es-MX', { style: 'currency', currency: 'MXN',}) }}*</td>
                </tr>
                ...
                <!--div style="position:absolute; font-size:14px; line-height: 1.6em;">
                    ...
                    <v-row style="padding:5px 5px 1px 5px; margin:15px 0px; background:#eeeeee;">
                        ...
                        <strong>Cuenta: </strong>025028060010<br/><strong>Clabe: </strong>058580250280600102
                        </div>
                    </v-row>
                </div-->
                <tr class="total">
                    <td colspan="3"></td>
                    <td>Total: {{ quotation.amount.toLocaleString('es-MX', { style: 'currency', currency: 'MXN',}) }}</td>
                </tr>
                ...
                <tr>
                    <td colspan="4" style="font-size:12px; line-height: 1.6em;" class="pt-10">
                        ...
                        <strong>
                            - Precios NO Inlcuyen IVA<br/>
                            - VIGENCIA DE COTIZACIÓN: Agosto 2022<br/>
                            - Tiempo de entrega: VARIABLE<br/>
                            - Garantía: 3 Meses</strong> (Desprendimiento/Defecto de Fabrica) (Suela Partida). 1 a 1 <br/>
                            <strong>- Aplica restricciones
                        </strong>
                        Quedo a sus órdenes para cualquier duda o comentario al respecto.
                    </td>
                </tr>
                <!--span style="font-size:12px; position:absolute;">Esta cotización tiene valides hasta el {{new Date(quotation.created_at).toLocaleDateString()}}</span-->
            </table>
            ...
            <div style="background:#363536; color:white; font-size:12px; line-height:1.5em; position:absolute; bottom:0; width: 644px;" class="mt-6 pa-4">
                <v-row class="px-6 pt-2 pb-5">
                    www.efiindustrial.com 
                    <v-divider style="border-color: transparent!important;"></v-divider>
                    <v-icon class="mr-3" small color="white">mdi-phone</v-icon>(81) 8359 7575
                </v-row>
                <v-row class="px-6 pb-2">
                    <v-icon class="mr-3" small color="white">mdi-map-marker</v-icon>Elías Reynoso Santana 314, Buenos Aires 64800, Monterrey, N.L.
                    <v-divider style="border-color: transparent!important;"></v-divider>
                    <v-icon class="mr-3" small color="white">mdi-whatsapp</v-icon>(81) 2474 0479
                </v-row>
            </div-->
        </div>

        <VueHtml2pdf :manual-pagination="true" :enable-download="true" ref="DownloadComp" :image="{ type: 'jpg', quality: 1 }">
...
export default {
    ...
    methods:{
        ...
        itemDetail(id){
            return this.$store.state.item.items.filter(item=>item.id == id)[0]//.map(item => item.name)[0]
        },
        companyName(id){
