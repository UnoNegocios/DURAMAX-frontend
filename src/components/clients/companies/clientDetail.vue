          ...
          <v-row style="max-width: 100vw; margin:auto;">
            ...
            <v-col class="px-4 py-7" cols="12" md="3" >
              ...
              <v-card class="px-6 pa-6 mt-4" outlined>
                ...
                <v-row>
                  <v-spacer></v-spacer>
                  <v-btn @click="saveNote()" :disabled="gris" :loading="gris" text>
                    GUARDAR
                  </v-btn>
                </v-row>
              </v-card>
...
import ActivityLog from "../../activitylog/container"
  export default {
    ...
    methods: {
      saveNote(){
        this.gris = true
        axios.put("https://backendduramax.unocrm.mx/api/v1/company/update",Object.assign(this.companyDetail)).then(response=>{
          this.gris = false
          this.$store.dispatch('company/getCompanies')
        })
      },
